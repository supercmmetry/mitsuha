    ...
    Context: Send + Sync + ChannelUtilityProvider,
{
    async fn store_compute_input(
        ...
        ctx.append_skip_channel_list(&mut input, state.bypass_channel_ids.clone()).await;
        ...
        + ChannelUtilityProvider,
{
    pub async fn new(
        ...
        let partition_repository = partition::service::Service::new(
            ...
            config.job.scheduler.core_scheduling_capacity.compute,
            max_shards,
        )
        .await;
        ...
        let job_queue_repository = job_queue::service::Service::new().await;
    ...
    async fn partition_poll_event_loop(state: &SchedulerState<Context>) -> types::Result<()> {
        ...
        state.partition_repository.remove_stale_partitions().await?;

        let mut partition_renewer: Option<JoinHandle<()>> = None;

        let ctx = Context::default();

        let should_renew_partition = partition_renewer.is_none()
            || partition_renewer.as_ref().is_some_and(|x| x.is_finished());
        if should_renew_partition {
            let repo = state.partition_repository.clone();
            let partition_id = partition_id.clone();

            let task = tokio::task::spawn_blocking(move || {
                _ = Handle::current().block_on(repo.renew_lease(partition_id.clone()));
            });

            partition_renewer = Some(task);
        }
            ...
            .await?
        {
            ...
            Self::mark_compute_input_as_queued(&mut input);
    ...
    async fn partition_poller(state: SchedulerState<Context>) -> types::Result<()> {
        let event_loop = async move {
            loop {
                if let Err(e) = Self::partition_poll_event_loop(&state).await {
    ...
    pub async fn schedule(
        ...
        if Self::is_compute_input_queued(&compute_input) {
            return Ok(true);
        }
