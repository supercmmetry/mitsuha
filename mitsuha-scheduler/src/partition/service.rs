...
use mitsuha_persistence::module::{ActiveModel as ModuleActiveModel, Entity as ModuleEntity, Model as ModuleModel, Column as ModuleColumn};
...
use sea_orm::{
    ...
    EntityName, EntityTrait, QueryFilter, QuerySelect, Statement, TransactionTrait, TryIntoModel,
};
use std::sync::Arc;
...
impl Service {
    ...
    async fn reshard_partitions_by_tx(&self, tx: &DatabaseTransaction) -> types::Result<()> {
        ...
        let sql = format!(
            "with numbered_rows as (select id, row_number() over (order by id) as row_num from {table_name}) update {table_name} join numbered_rows on {table_name}.id = numbered_rows.id set shard_start = (row_num - 1) * ({max_shards} / (select count(*) from {table_name})), shard_end = row_num * ({max_shards} / (select count(*) from {table_name})) - 1;",
...
impl Repository for Service {
    ...
    async fn remove(&self, id: String) -> types::Result<()> {
        ...
        if existing_partition.lease_expiry.timestamp()
            + self.partition_lease_duration_skew.num_seconds()
            >= Utc::now().timestamp()
        {
            return Err(Error::EntityConflictError {
                name: id.clone(),
                kind: KIND.clone(),
                reason: "the partition has not expired or has just expired recently, cannot delete"
                    .to_string(),
            });
        }
