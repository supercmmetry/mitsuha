...
use wasi_common::file::{Advice, FdFlags, Filestat, FileType};
...
impl File {
    pub async fn new(fs: AsyncNativeFileSystem, path: PathBuf, open_options: OpenOptions) -> Result<Self, Error> {
        let absolute_cow_path = path.absolutize_virtually("/").map_err(|e| Error::trap(e.into()))?;
        let absolute_path = absolute_cow_path.deref();

        let obj = Self {
            fs: Arc::new(fs.clone()),
            file: Arc::new(RwLock::new(AsyncFile::new(fs, absolute_path.to_string().map_err(|e| Error::trap(e))?))),
    ...
    async fn open(&self, open_options: OpenOptions) -> Result<(), Error> {
        if open_options.create {
            if !self.fs.exists(self.path.as_path()).await.map_err(|e| Error::trap(e))? {
                self.fs.create_empty_file(self.path.as_path()).await.map_err(|e| Error::trap(e))?;
            }
        ...
        if open_options.truncate {
            self.fs.truncate(self.path.as_path(), 0u64).await.map_err(|e| Error::trap(e))?;
        }
    ...
    async fn get_filestat_(&self) -> Result<Filestat, Error> {
        ...
        let size = self.fs.get_file_size(path).await.map_err(|e| Error::trap(e))?;
    ...
    async fn set_len(&self, len: u64) -> Result<(), Error> {
        self.fs.truncate(self.path.as_path(), len).await.map_err(|e| Error::trap(e))
    }
    ...
    async fn set_fdflags_(&self, flags: FdFlags) -> Result<(), Error> {
        *self.fdflags.write().await = flags;
        ...
        if flags.intersects(FdFlags::APPEND) {
            self.file.write().await.seek(SeekFrom::End(0)).await.map_err(|e| Error::trap(e.into()))?;
        }
...
impl WasiFile for File {
    ...
    async fn set_times(&self, _atime: Option<SystemTimeSpec>, _mtime: Option<SystemTimeSpec>) -> Result<(), Error> {
        Ok(())
    }
